...
import org.edupro.webapi.academic.service.AcademicSessionService;
...
public class AcademicSessionController extends BaseController<AcademicSessionRes> {
    ...
    @PostMapping
    private ResponseEntity<Response> save(@RequestBody @Valid AcademicSessionReq request){
        var result = service.save(request);
        return getResponse(result);
    }

    @PutMapping("/{id}")
    private ResponseEntity<Response> update(@RequestBody @Valid AcademicSessionReq request,
